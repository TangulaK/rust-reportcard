...
var (
	...
	skipSuffixes   = []string{".pb.rs", ".pb.gw.rs", ".generated.rs", "bindata.rs", "_string.rs"}
...
type Children struct {
	...
	Rendered interface{}   `json:"rendered"`
...
type Spans struct {
	...
	SuggestedReplacement    interface{} `json:"suggested_replacement"`
	SuggestionApplicability interface{} `json:"suggestion_applicability"`
	Text                    []Text      `json:"text"`
}
type Message struct {
...
// AddError2 adds an Error to FileSummary
func (fs *FileSummary) AddError2(msg string, ln int) error {
	// s := strings.SplitN(out, ":", 2)
	// msg := strings.SplitAfterN(s[1], ":", 3)[2]
...
outer:
	for out.Scan() {
		...
		if err := json.Unmarshal([]byte(stdOut), &message); err != nil {
			continue outer
		}
		if message.Reason == "compiler-message" && len(message.Message.Spans) > 0 {
			filename := fmt.Sprintf("%s:%d:%d", message.Message.Spans[0].FileName, message.Message.Spans[0].LineStart, message.Message.Spans[0].LineEnd)
			...
			err := fs.AddError2(message.Message.Rendered, message.Message.Spans[0].LineStart)
			if err != nil {
				return nil, err
			}
			fsMap[filename] = fs
