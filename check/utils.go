...
outer:
	for out.Scan() {
		...
		if message.Reason == "compiler-message" && len(message.Message.Spans) > 0 {
			...
			err := fs.AddError2(message.Message.Message, message.Message.Spans[0].LineStart)
			if err != nil {
				return nil, err
			}
			fsMap[filename] = fs
